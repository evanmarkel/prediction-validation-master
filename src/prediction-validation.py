...
def main(window, actual_price, predicted_price, output_error):
    #read in window size from input file
    window_size = None
    with open(window, 'r') as file: window_size = int(file.read())
    ...
    actual_df = pd.read_csv(actual_price, delimiter='|', header=None, index_col=['hour','ID'],
                            names=['hour','ID','price'], skip_blank_lines=True, float_precision='high')
    ...
    combined_df['rolling_error'] = combined_df['abs_error'].rolling(window_size, min_periods=1).mean()
    ...
    output_df[['hour', 'end_hour', 'avg_error']].to_csv(output_file,delimiter='|', header=None, na_rep='NA', float_format='%.2f')
